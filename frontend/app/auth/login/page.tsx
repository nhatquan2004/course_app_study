...
function LoginPage() {
	...
	// 			document.cookie = `loginToken=${data.token}; path=/; max-age=3600, SameSite=Lax`;
	// 			document.cookie = `userRole=${data.role}; path=/; max-age=3600; SameSite=Lax`;
	// 			router.push('/');
	// 		} else {
	// 			setMessage(data.message || 'Đăng nhập không thành công.');
	// 		}
	// 	} catch (err) {
	// 		console.error('Error handle login:', err);
